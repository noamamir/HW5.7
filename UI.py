...
class UI(tk.Tk):
    def __init__(self):
        ...
        self.polygon_list = []
        self.path_list= []
        self.picking_start_point = 0
        self.picking_end_point = 0
        self.pathfinder_activated= 0
            ...
            for i in range(self.polyline_number):
                for polygon in self.polygon_list:
                    ...
                    if polygon.isInside(self.points[start_vertex+self.vertex_number[i]]):
                        t_polygon = polygon
                self.path_list.append(Route(s_polygon,t_polygon, i, self.points[start_vertex:(start_vertex+self.vertex_number[i])]))
    ...
    def load_polygons(self):
        ...
        for i in range(len(names)):
            ...
            for vertex in range(num_of_vertices[i]):
                points.append(Point2D(coordinate_pairs[already_read_vertices][0], coordinate_pairs[already_read_vertices][1], vertex))
    ...
    def pick_start_point(self):
        if self.chose_start_point == 1:
            messagebox.showerror("Error", "To choose a different start point, hit Refresh")
        self.picking_start_point = 1
        self. picking_end_point = 0
        self.map_widget.add_left_click_map_command(self.left_click_event)
        self.chose_start_point = 1

    def pick_end_point(self):
        if self.chose_end_point == 1:
            messagebox.showerror("Error", "To choose a different end point, hit Refresh")
        self.picking_start_point = 0
        self. picking_end_point = 1
        self.map_widget.add_left_click_map_command(self.left_click_event)
        self.chose_end_point = 1

    def pathfinder(self):
        """Calling the pathfinder function, which will also mark the shortest path on the map"""
        self.pathfinder_activated =1
        self.graph[0].shortestPath(self.start_polygon[0], self.end_polygon[0], self.map_widget)
